...
class yahoo_finance:
    ...
    def __call__(self,stock:str, start_date:datetime=None, end_date:datetime=None, method:ScrapeMethod=ScrapeMethod.ALL) -> bool:
        ...
        if self.method == ScrapeMethod.ALL:
            self.start_date:datetime=helper.dt.get_date_n_ago(helper.dt.get_today_date(),period=40,freq="y")
            self.end_date:datetime=helper.dt.get_today_date()
    ...
    def scrape(self):
        ...
        self.request_info.update(helper.dt.get_date_n_ago(self.end_date, period=6, freq="m"), self.end_date)
    ...
    def transform(self):
        ...
        helper.dt.to_datetime(self.response_info.data[Columns.TIMESTAMP.value[0]])
        ...
        #New request
        self.request_info.update(start_date= helper.dt.get_date_n_ago(self.response_info.start_date,period=6, freq="m"),
                                 end_date= helper.dt.get_date_n_ago(self.response_info.start_date,period=1,freq="d"))
